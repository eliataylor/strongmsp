...
export const OAFormPromptTemplates: React.FC<OAFormProps<"PromptTemplates">> = ({ onSuccess }) => {

  const { renderField, handleSubmit, handleDelete, errors, navItem, entity, syncing } = useForm<"PromptTemplates">();
  ...
  async function saveEntity() {
    handleSubmit().then((newentity) => {
      ...
      enqueueSnackbar(`PromptTemplates saved`);
    }).catch(error => {
      console.error(error);
  ...
  return (
    <AlternatingList container spacing={4} p={1} justifyContent={'space-between'} wrap={"wrap"} >
      			<Grid item xs={12} >
				{renderField(TypeFieldSchema["PromptTemplates"]["prompt"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["PromptTemplates"]["instructions"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["PromptTemplates"]["model"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["PromptTemplates"]["status"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["PromptTemplates"]["purpose"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["PromptTemplates"]["response_format"], 0, {fullWidth:true})}
			</Grid>
...
export default OAFormPromptTemplates;
