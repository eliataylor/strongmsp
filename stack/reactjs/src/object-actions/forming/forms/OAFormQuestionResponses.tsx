...
export const OAFormQuestionResponses: React.FC<OAFormProps<"QuestionResponses">> = ({ onSuccess }) => {

  const { renderField, handleSubmit, handleDelete, errors, navItem, entity, syncing } = useForm<"QuestionResponses">();
  ...
  async function saveEntity() {
    handleSubmit().then((newentity) => {
      ...
      enqueueSnackbar(`QuestionResponses saved`);
    }).catch(error => {
      console.error(error);
  ...
  return (
    <AlternatingList container spacing={4} p={1} justifyContent={'space-between'} wrap={"wrap"} >
      			<Grid item xs={12} >
				{renderField(TypeFieldSchema["QuestionResponses"]["author"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["QuestionResponses"]["question"], 0, {fullWidth:true})}
			</Grid>
			<Grid item xs={12} >
				{renderField(TypeFieldSchema["QuestionResponses"]["response"], 0, {fullWidth:true})}
			</Grid>
...
export default OAFormQuestionResponses;
